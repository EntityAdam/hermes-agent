...
Currently Matrix is expected to be available on Linux, macOS, and Windows.
"""



class TestMatrixPlatformVisibility:
    ...
    def test_matrix_present_on_windows(self, monkeypatch):
        import hermes_cli.gateway as gateway_mod

        monkeypatch.setattr(gateway_mod.sys, "platform", "win32")
        platforms = gateway_mod._all_platforms()
        keys = {p["key"] for p in platforms}
        assert "matrix" in keys, "matrix must be available on Windows"
